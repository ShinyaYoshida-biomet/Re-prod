...
pub async fn handle_project_request(
    ...
    match request {
        ...
        WSRequest::ProjectAddExisting { path } => match state.projects.add_existing_project(Path::new(path)).await {
            Ok(descriptor) => Some(
                switch_runtime(
                    state,
                    current_runtime,
        ...
        WSRequest::ProjectStateLoad { project_id } => Some(match state.projects.load_state(project_id).await {
            Ok(state_payload) => {
                send_responses(
                    socket,
        ...
        WSRequest::ProjectStateSave {
            project_id,
            state: payload,
        } => Some(match state.projects.save_state(project_id, payload.clone()).await {
            Ok(()) => {
                send_responses(
                    socket,
