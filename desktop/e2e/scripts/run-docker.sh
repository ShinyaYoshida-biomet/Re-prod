...
PRE_COMMAND='toolchain_marker="/root/.cache/reprod-e2e-toolchain-ok"; if [[ ! -f "$toolchain_marker" ]]; then collect2_path=$(gcc -print-file-name=collect2 2>/dev/null || true); openssl_pc=""; for candidate in /usr/lib/*/pkgconfig/openssl.pc /usr/lib/pkgconfig/openssl.pc; do if [[ -e "$candidate" ]]; then openssl_pc="$candidate"; break; fi; done; if [[ ( -n "$collect2_path" && -f "$collect2_path" && ! -s "$collect2_path" ) || -z "$openssl_pc" || ! -s "$openssl_pc" ]]; then apt-get update && apt-get install --reinstall -y gcc-11 g++-11 libssl-dev pkg-config && rm -rf /var/lib/apt/lists/* && touch "$toolchain_marker"; else touch "$toolchain_marker"; fi; fi;'
